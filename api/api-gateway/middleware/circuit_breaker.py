...
class CircuitBreaker:
    ...
    def record_failure(self, service_name: str) -> None:
        ...
        if circuit["state"] == CircuitState.CLOSED and circuit["failure_count"] >= self.failure_threshold:
    ...
    def is_circuit_open(self, service_name: str) -> bool:
        ...
        if circuit["state"] == CircuitState.OPEN:
            ...
            if circuit["last_failure_time"] and datetime.utcnow() - circuit["last_failure_time"] > timedelta(seconds=self.recovery_timeout):
    ...
    async def call_service(self, service_name: str, url: str, method: str, **kwargs) -> httpx.Response:
        """
...
async def circuit_breaker_middleware(request: Request, call_next: Callable[[Request], Awaitable[JSONResponse]]) -> JSONResponse:
    """
