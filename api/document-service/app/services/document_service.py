...
from api.document-service.app.schemas.document import (
    DocumentCreateDTO,
    DocumentUpdateDTO,
    DocumentResponseDTO,
    DocumentVersionDTO,
...
from api.document-service.app.factories.document_factory import DocumentFactory
...
class DocumentService:
    ...
    def create_document(self, document_data: DocumentCreateDTO, user_id: str) -> DocumentResponseDTO:
        """
        ...
        if document_data.parent_id:
            parent_document = self.db.query(Document).filter(
    ...
    def get_document(self, document_id: str, user_id: str) -> DocumentResponseDTO:
        ...
        if not self._has_permission(document_id, user_id, "view"):
            raise InsufficientDocumentPermissionException("User does not have permission to view this document")
        
        # Return document
        return self._document_to_dto(document)
    
    def update_document(self, document_id: str, document_data: DocumentUpdateDTO, user_id: str) -> DocumentResponseDTO:
        """
        Update a document.
        ...
        if not self._has_permission(document_id, user_id, "edit"):
            raise InsufficientDocumentPermissionException("User does not have permission to edit this document")
    ...
    def delete_document(self, document_id: str, user_id: str) -> Dict[str, Any]:
        ...
        if not self._has_permission(document_id, user_id, "delete"):
            raise InsufficientDocumentPermissionException("User does not have permission to delete this document")
    ...
    def get_project_documents(self, project_id: str, user_id: str, parent_id: Optional[str] = None) -> List[DocumentResponseDTO]:
        """
    ...
    def upload_document(self, document_data: DocumentCreateDTO, user_id: str) -> DocumentUploadResponseDTO:
        """
        Upload a document.
        ...
        try:
            ...
            self.db.delete(self.db.query(Document).filter(Document.id == document.id).first())
            self.db.commit()
            
            raise DocumentStorageException(f"Failed to generate upload URL: {e}")
    
    def create_document_version(self, document_id: str, content_type: str, changes: str, user_id: str) -> DocumentVersionDTO:
        """
        ...
        # Check if document type is file
        if document.type != DocumentType.FILE:
            raise InvalidDocumentTypeException("Document type must be file for versioning")
    ...
    def get_document_versions(self, document_id: str, user_id: str) -> List[DocumentVersionDTO]:
        """
        ...
        document_versions = self.db.query(DocumentVersion).filter(
            DocumentVersion.document_id == document_id
        ).order_by(
            DocumentVersion.version.desc()
        ).all()
    ...
    def get_document_version(self, document_id: str, version: int, user_id: str) -> DocumentVersionDTO:
        """
        ...
        document_version = self.db.query(DocumentVersion).filter(
            DocumentVersion.document_id == document_id,
    ...
    def add_document_permission(self, document_id: str, permission_data: DocumentPermissionCreateDTO, user_id: str) -> DocumentPermissionDTO:
        """
        ...
        if not self._has_permission(document_id, user_id, "share"):
            raise InsufficientDocumentPermissionException("User does not have permission to share this document")
    ...
    def update_document_permission(self, document_id: str, permission_id: str, permission_data: DocumentPermissionUpdateDTO, user_id: str) -> DocumentPermissionDTO:
        """
        ...
        document_permission = self.db.query(DocumentPermission).filter(
        ...
        document_permission.updated_at = datetime.utcnow()
        self.db.commit()
        self.db.refresh(document_permission)
    ...
    def delete_document_permission(self, document_id: str, permission_id: str, user_id: str) -> Dict[str, Any]:
        """
        ...
        if document_permission.user_id == document.creator_id:
            raise InsufficientDocumentPermissionException("Cannot delete owner's permission")
    ...
    def get_document_permissions(self, document_id: str, user_id: str) -> List[DocumentPermissionDTO]:
        """
        ...
        document_permissions = self.db.query(DocumentPermission).filter(
            DocumentPermission.document_id == document_id
        ).all()
        
        # Return document permissions
        return [self._document_permission_to_dto(permission) for permission in document_permissions]
    ...
    def _has_permission(self, document_id: str, user_id: str, permission_type: str) -> bool:
        ...
        # Get user's roles
        user_roles = self.db.query("user_roles").filter(
        ...
        if project_member and project_member.role in ["owner", "admin"]:
            return True
    ...
    def _document_version_to_dto(self, document_version: DocumentVersion) -> DocumentVersionDTO:
        """
    ...
    def _document_permission_to_dto(self, document_permission: DocumentPermission) -> DocumentPermissionDTO:
        """
